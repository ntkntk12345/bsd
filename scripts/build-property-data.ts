  ...
  while (true) {
    try {
      ...
      if (!hasLoggedWait) {
        hasLoggedWait = true;
        console.warn(
          `[property-data] Another build appears to be running (lock exists at ${BUILD_LOCK_DIR}). Waiting...`,
        );
      }

      if (Date.now() >= deadline) {
        throw new Error(
          `Timed out waiting for property data build lock at ${BUILD_LOCK_DIR}. ` +
            `If no other build is running, delete this folder and re-run.`,
        );
      }
